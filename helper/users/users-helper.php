...
if ( ! class_exists( 'DDWCWM_Users_Helper' ) ) {
	...
	class DDWCWM_Users_Helper {
		...
		/**
		 * Get user wallet withdraw requests functions
		 * 
		 * @param mixed $user_id
		 * @return array
		 */
		public function ddwcwm_get_user_withdraw_requests( $user_id = '' ) {
			if ( ! empty( $user_id ) ) {
				$this->user_id = $user_id;
			}

			$withdraw_requests = get_user_meta( $this->user_id, '_ddwcwm_wallet_withdraw_requests', true );

			return ! empty( $withdraw_requests ) ? $withdraw_requests : [];
		}

		/**
		 * Set user wallet withdraw requests functions
		 * 
		 * @param mixed $user_id
		 * @return float
		 */
		public function ddwcwm_set_user_withdraw_requests( $withdraw_requests, $user_id = '' ) {
			if ( ! empty( $user_id ) ) {
				$this->user_id = $user_id;
			}

			update_user_meta( $this->user_id, '_ddwcwm_wallet_withdraw_requests', $withdraw_requests );
		}

		/**
		 * Delete user wallet withdraw requests functions
		 * 
		 * @param mixed $user_id
		 * @return float
		 */
		public function ddwcwm_delete_user_withdraw_requests( $user_id = '' ) {
			if ( ! empty( $user_id ) ) {
				$this->user_id = $user_id;
			}

			delete_user_meta( $this->user_id, '_ddwcwm_wallet_withdraw_requests' );
		}

		/**
		 * Get user wallet money requests function
		 * 
		 * @param mixed $user_id
		 * @return array
		 */
		public function ddwcwm_get_user_money_requests( $user_id = '' ) {
			if ( ! empty( $user_id ) ) {
				$this->user_id = $user_id;
			}

			$money_requests = get_user_meta( $this->user_id, '_ddwcwm_wallet_money_requests', true );

			return ! empty( $money_requests ) && is_array( $money_requests ) ? $money_requests : [];
		}

		/**
		 * Set user wallet money requests function
		 * 
		 * @param array $money_requests
		 * @param mixed $user_id
		 * @return void
		 */
		public function ddwcwm_set_user_money_requests( $money_requests, $user_id = '' ) {
			if ( ! empty( $user_id ) ) {
				$this->user_id = $user_id;
			}

			update_user_meta( $this->user_id, '_ddwcwm_wallet_money_requests', $money_requests );
		}
